...
def privacy_keyboard():
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Да, ознакомил(ась)", callback_data="privacy_yes")],
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "Перед началом работы ознакомьтесь с политикой обработки персональных данных: "
...
async def ask_support(message: Message, state: FSMContext):
    ...
    await message.reply(
        ...
        "Сообщение будет отправлено нашему менеджеру ЗАБУГОРНЫЙLUX.",
        parse_mode="HTML",
        reply_markup=types.ReplyKeyboardRemove()
    )
    await state.set_state(SupportState.waiting_support_message)

@dp.message(SupportState.waiting_support_message)
async def process_support_message(message: Message, state: FSMContext):
        ...
        try:
            await bot.send_message(
                ...
                reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                    [InlineKeyboardButton(text="💌 Ответить", callback_data=f"admin_msg:{user_id}")]
                ]),
                parse_mode="HTML"
            )
        except Exception as e:
    ...
    await message.answer(
        "✅ <b>Спасибо!</b>\n\n"
        "Ваше сообщение отправлено. Наш менеджер ЗАБУГОРНЫЙLUX свяжется с Вами.",
        reply_markup=main_user_keyboard(),
        parse_mode="HTML"
    )
...
async def delete_request(cb: CallbackQuery):
    req_id = int(cb.data.split(":", 1)[1])
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("UPDATE requests SET status = 'deleted' WHERE id = ?", (req_id,))
        await db.commit()
...
async def start_admin_message(cb: CallbackQuery, state: FSMContext):
    ...
    await bot.send_message(
        cb.from_user.id,
        f"💌 <b>Напишите сообщение для пользователя</b>\n\n"
        f"ID получателя: <code>{user_id}</code>\n\n"
        f"Отправьте /cancel для отмены",
        parse_mode="HTML"
    )

@dp.message(AdminState.waiting_admin_message, Command("cancel"))
...
async def send_admin_message(message: Message, state: FSMContext):
    ...
    try:
        await bot.send_message(
            target_user_id,
            f"📨 <b>Сообщение от менеджера ЗАБУГОРНЫЙLUX:</b>\n\n{message.text}",
            parse_mode="HTML"
        )
        await message.answer(f"✅ Сообщение отправлено пользователю {target_user_id}")
    except Exception as e:
        logger.error("Failed to send message to user %s: %s", target_user_id, e)
...
async def show_stats(message: Message):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute("SELECT COUNT(*) FROM requests WHERE status = 'new'")
        new_count = (await cursor.fetchone())[0]
        
        cursor = await db.execute("SELECT COUNT(*) FROM requests WHERE status = 'in_progress'")
        progress_count = (await cursor.fetchone())[0]
        
        cursor = await db.execute("SELECT COUNT(*) FROM requests WHERE status = 'deleted'")
...
async def show_all_requests(message: Message):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute(
            "SELECT id, name, phones, brand_model, status FROM requests ORDER BY id DESC LIMIT 20"
        )
    ...
    for req in requests:
        ...
        text += f"{status_emoji} <b>#{req[0]}</b> - {req[1]} ({req[3]})\n"
    
    await message.answer(text, parse_mode="HTML")

@dp.message(Command("new_requests"))
async def show_new_requests(message: Message):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute(
            "SELECT id, user_id, name, phones, username, brand_model, exterior, interior, package, budget, year, wishes "
            "FROM requests WHERE status = 'new' ORDER BY id DESC"
        )
        requests = await cursor.fetchall()
    ...
    for req in requests:
        req_id, user_id, name, phones, username, brand_model, exterior, interior, package, budget, year, wishes = req
        msg_text = (
            f"🆕 <b>Заявка #{req_id}</b>\n\n"
            f"👤 <b>ФИО:</b> {name}\n"
            f"☎️ <b>Телефоны:</b> {phones}\n"
            f"👤 <b>Username:</b> {username}\n"
            f"🚗 <b>Марка/модель:</b> {brand_model}\n"
        ...
        try:
            await message.answer(
                msg_text,
                reply_markup=admin_request_kb(req_id, phones, user_id),
                parse_mode="HTML"
            )
        except Exception as e:
            logger.error("Failed to send request #%s: %s", req_id, e)

@dp.message(Command("in_progress"))
async def show_in_progress_requests(message: Message):
    if message.from_user.id not in ADMINS:
        return
    
    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute(
            "SELECT id, name, phones, brand_model FROM requests WHERE status = 'in_progress' ORDER BY id DESC"
        )
        requests = await cursor.fetchall()
    ...
    text = "⚙️ <b>Заявки в работе:</b>\n\n"
    for req in requests:
        text += f"<b>#{req[0]}</b> - {req[1]} ({req[3]})\n"
